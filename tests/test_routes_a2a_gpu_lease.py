...
class TestClusterLeaseIntegration:
    ...
    async def test_release_does_not_free_another_holders_lease(
        ...
        # The node-scoped release frees only the caller's OWN claim; an agent's
        # `a2a:` lease must not be freed by an admin who merely presents the
        # agent's canonical id in the body's `holder=` field (CR on #2988).
        ...
        # Node-scoped release with `holder=<agent>` must NOT free the lease.
        resp = await lease_client.post(
            "/api/a2a/gpu/release",
        ...
        # The operator path (explicit id) still works and attributes the line
        # to the freed holder.
        ...
        assert released.json()["lease_id"] == lease_id
        assert cluster.get_leases() == []
