...
class TestMkdocsExcludeReplacement:
    def test_build_succeeds_without_mkdocs_exclude_plugin(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            built_html = list(site_dir.rglob("*.html"))
            built_names = [p.name for p in built_html]
            ...
            for forbidden in [
                ...
                assert not any(
                    forbidden in name for name in built_names
                ), f"Excluded page '{forbidden}' found in built site: {built_names}"
