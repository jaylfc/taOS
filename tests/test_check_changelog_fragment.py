...
class TestIsTestPath:
    ...
    def test_frontend_test_files(self):
        ...
        assert _is_test_path("desktop/src/__tests__/bar.test.js") is True
...
class TestMainIntegration:
    ...
    def _mock_git_log(self, mock_run, commit_messages):
        """Helper to mock git log for commit messages."""
        mock_result = MagicMock()
        mock_result.stdout = "\x00".join(commit_messages) + "\x00"
        mock_run.return_value = mock_result
    ...
    def test_main_fails_on_non_test_change_no_fragment(self):
        ...
        with patch.object(_MOD.subprocess, "run") as mock_run:
            # First call: diff, second call: log
            mock_diff = MagicMock()
            mock_diff.stdout = "M\x00tinyagentos/routes/project_invites.py\x00"
            mock_log = MagicMock()
            mock_log.stdout = "Initial commit\x00"
            mock_run.side_effect = [mock_diff, mock_log]
    ...
    def test_main_passes_with_fragment(self):
        ...
        with patch.object(_MOD.subprocess, "run") as mock_run:
            ...
            mock_log = MagicMock()
            mock_log.stdout = "Add feature\x00"
            mock_run.side_effect = [mock_diff, mock_log]
    ...
    def test_main_passes_with_pr_label_escape(self):
        """Main exits 0 when PR has no-changelog-needed label."""
        with patch.object(_MOD.subprocess, "run") as mock_run:
            mock_diff = MagicMock()
            mock_diff.stdout = "M\x00tinyagentos/routes/project_invites.py\x00"
            mock_log = MagicMock()
            mock_log.stdout = "Refactor\x00"
            mock_run.side_effect = [mock_diff, mock_log]
