...
again. The blessed mechanism is the **S1 hook** (`tsk-vp7tt6`): a shared hook
that listens for `focus` and `visibilitychange` events and triggers a refetch.
Fetch-on-mount-once is forbidden.

## 2. Push updates via the OS event stream

Every surface MUST subscribe to the OS event stream for push updates. The
blessed mechanism is the **S2 hook** (`tsk-yh36f7`): a shared hook that opens
the SSE connection and dispatches events to the surface's state layer. Polling
is a fallback only when the stream is unavailable, never the primary path.
...
the shared S1/S2 hooks, add the missing transitions. The only exception is when
doing so dwarfs the original change; in that case the PR description must name
the gap and a card must exist for it.
