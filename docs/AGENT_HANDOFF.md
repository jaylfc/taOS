...
> 6. (Claude Code only) read `~/.claude/.../memory/MEMORY.md`.
>
...
- Conventional commits preferred: `fix(scope): description`, `feat(scope): description`, `docs: description`, `chore(scope): description`.
...
Fork PRs receive no automated review. A maintainer reviews them by hand. Lead review IS the gate: the `bot-review-gate` check stays red until a maintainer approves or applies `lead-reviewed`. `scripts/check_bot_review.py` exits `EXIT_FORK_UNREVIEWED` (3) when a fork PR has no maintainer approval and no `lead-reviewed` label. `bot-review-allow` does NOT waive this verdict. Expected turnaround is the same as in-repo PRs.
...
- `3 FAIL` when a fork PR has no maintainer approval and no `lead-reviewed` label.
