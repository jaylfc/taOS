...
▶ AGENT OS CONTROL FRAMEWORK = COMPLETE + DEPLOYED. #877 transport + #878 tools (open_app/arrange_windows + 09-os-control manual) MERGED. The agent can drive the desktop. #879 purge-purple-from-dark MERGED. Both DEPLOYED to Pi (dev 4f0f1957) + dark verified (no purple, badges/board cyan). REMAINING PR: #880 mobile-chat polish (avatars+tokens+timestamps+preview; /chat-pwa = chromeless mobile Messages, mirrors it) BAKING with review fixes (avatar-gate dm-only, ticking time) -> merge when green -> REDEPLOY Pi -> VERIFY /chat-pwa mobile LIGHT+DARK (the light-sensitive piece). NEXT framework step (optional): Phase 3 data tools (create_project/add_task/canvas_add_image wrap existing routes) for the storybook demo.
