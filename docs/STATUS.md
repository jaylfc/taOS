...
**Last updated:** 2026-06-11 ~16:25 BST, by @taOS (Mac session). PRIORITY (Jay): PR #786 fixes HarMaximus #783 rknpu install (binutils/strings missing made install-rknpu.sh die at the librknnrt step before rkllama ever installed, so /api/pull got connection-refused). #786 in CI; on green: merge dev, promote dev->master (carries #785 Phase 4 + #786), then REPLY on #783 with root cause + re-run instruction (tracked task #7). #781 + #785 already merged + #784 promoted. Usage 75% resets 22:20Z; tripwire + resume pair armed via the freshness cron.
