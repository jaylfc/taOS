...
Three rows in the original table were wrong against `origin/dev` (verified 2026-09-06):

- **S2-10**: the Fix row originally prescribed `UMask=0077` on `tinyagentos.service`. That is wrong: the controller runs as `taos` while backend units written by `scripts/install-llama-cpp.sh` / `install-hailo.sh` / `install-rk*.sh` run as `$SUDO_USER` and read the model store under `data/`, and `download_manager.py:430` writes with a bare `open('wb')`, so a 0077 umask makes every new model file 0600 and unreadable to the backend. Per-file modes (0700 dir, `atomic_write_text(mode=0o600)`) are the mechanism; `PrivateTmp=yes` stays.

- **S2-12**: the premise was wrong. `app.py:1700` puts agent workspaces at `data_dir/agent-workspaces`; the `/data/workspace` mount (`app.py:1849`) serves `data_dir/workspace`, which never contained agent files. The real exposure is the shared `images/generated` + `music/generated` directory (`routes/images.py:57,:62`; `routes/music.py:46`) with guessable filenames. Carded as **tsk-tfje2m**.

- **S2-21**: `{secret_key}` is substituted only in manifest `config_files` (`docker_installer.py:80-88`), not in `install.env` (`:127-128`, compose written `:175`). Using the placeholder in `install.env` ships the literal string. Carded as **tsk-teaogm**.
...
| S2-10 | `tinyagentos/llm_proxy.py:93` (default `/tmp/taos-litellm`), `:173,:185,:187-197` (master key from `litellm_config.py:394`, backend keys `:263-267`, imported shim modules); `app.py:394-401` passes no config dir; `download_manager.py:430` bare `open('wb')` | The proxy config, the LiteLLM master key and every backend key are written at umask mode into a world-shared, pre-creatable `/tmp` directory; the shim `.py` files there are imported by the proxy. Unit has neither `PrivateTmp` nor `UMask`. A `UMask=0077` on the unit would make every new model file 0600, but the controller runs as `taos` while backend units written by `scripts/install-llama-cpp.sh` / `install-hailo.sh` / `install-rk*.sh` run as `$SUDO_USER` and read the model store under `data/`, so 0600 files are unreadable to the backend. Same pattern at `routes/system_logs.py:129` and `scripts/pre-beta-to-beta.sh:368-373`. | **PROVEN** (mode 664 on the written files) | C SC2 | `<data_dir>/litellm/` at 0700, `atomic_write_text(mode=0o600)`, `PrivateTmp=yes`. Per-file modes are the mechanism; do NOT prescribe `UMask=0077` on the unit. |
...
| S2-12 | `tinyagentos/routes/images.py:57,:62`, `tinyagentos/routes/music.py:46` (`/data/images/generated`, `/data/music/generated`) | Authenticated but not user-scoped: any user reads any other user's generated images and music files under predictable paths. The `/data/workspace` mount (`app.py:1849`) serves `data_dir/workspace`, which never contained agent files (`app.py:1700` puts agent workspaces at `data_dir/agent-workspaces`). | review-only | B SB3 | Replace the mount with a route that resolves the owner from the first path segment and applies `require_owner_or_admin`. See **tsk-tfje2m**. |
...
| S2-21 | `app-catalog/services/linkwarden/manifest.yaml:21-24` | `{secret_key}` is substituted only in manifest `config_files` (`docker_installer.py:80-88`), not in `install.env` (`:127-128`, compose written `:175`). Using the placeholder in `install.env` ships the literal string `{secret_key}`. `DATABASE_URL` points at a Postgres the manifest never starts. | review-only | D S3 | Use `{secret_key}` in `config_files` only; add the Postgres service or drop the URL. See **tsk-teaogm**. |
...
`{secret_key}` provisioning in `docker_installer.py` (substituted in manifest `config_files` at `:80-88`, NOT in `install.env` at `:127-128` — using it there ships the literal string).
