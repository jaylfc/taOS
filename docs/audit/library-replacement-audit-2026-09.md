...
| Fix the eval detector's lookbehind | `code_analyzer.py:135` — also match `\b(?:window\|globalThis\|self)\.eval\s*\(` | Closes an indirect-eval hole in a security gate (S8) |
...
- ~~**Route the five byte-size parsers at the one correct implementation**
  ...
  `_parse_memory("512m") → 0` and the never-enforced quota above 1 TiB (R1).~~ **Done (tsk-j67fpt):**
  all five now call `tinyagentos/size_units.py`, which carries the stdlib unit table this row
  recommended over `humanfriendly`; `disk_quota` reaches PiB and logs a warning on a parse miss
  rather than returning `None`. The eight JS formatters are untouched — R1's desktop half is
  still open.
...
- **Backport `routes/peer.py`'s `_RATE_HITS_MAX_SIZE` eviction** into `rate_limit.py` and
  `auth_middleware.py`, and switch the fixed windows to `time.monotonic()` (S5).
