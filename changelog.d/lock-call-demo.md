- Lock screen: an incoming-call demo in the live zone. A call from Naira gathers
  ...
  PA finishes, the card shrinks to a one-line "Reminder added · Call Naira ·
  5:30 pm" pill, the feed returns exactly as it was, and the PA's reminder
...
- Lock screen: the call is scripted demo content served by `/auth/lock-call`
  ...
  independent of the other demo flags. There is no path from it to a phone
