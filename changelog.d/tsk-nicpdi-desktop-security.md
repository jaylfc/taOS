...
- Install retry: installation completion is tracked on its own flag, so a transient apt failure no longer permanently skips installation on every later request; `start` is rejected with 409 while installation has not completed
