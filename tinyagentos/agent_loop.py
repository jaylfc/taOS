...
class AgentLoop:
    ...
    async def spawn_subagent(
        ...
        task_obj = asyncio.create_task(_runner(), name=f"subagent:{sub_id}")
    ...
    async def reach_safe_point(self) -> list[QueuedMessage]:
        ...
        If any queued message is a redirect, :meth:`cancel_subagents` is
        invoked first so in-flight subagent work is aborted before the next
        turn starts -- but cancellation happens here, at the safe boundary,
        never mid-step.
    ...
    async def await_all_subagents(self, timeout: float | None = None) -> None:
        ...
        Does not cancel them -- just waits. Useful before the loop can
        safely reach a safe point and go idle.
        """
        async with self._lock:
            tasks = [
                e.task_obj
