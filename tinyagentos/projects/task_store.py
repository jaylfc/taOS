...
class ProjectTaskStore(BaseStore):
    ...
    async def update_checklist_item(
        ...
        *,
        ...
        await self._db.execute(
            f"UPDATE task_checklist_items SET {', '.join(sets)} WHERE id = ?",
            params,
        )
        await self._db.commit()
        return await self.get_checklist_item(item_id)
    ...
    async def archive_checklist_item(self, item_id: str, reported_by: str) -> dict:
        ...
        Raises ValueError if the item is missing, or if it lacks verification
        or a report.  Mirrors sibling task mutations by publishing
        ``checklist.item.archived`` under the task's resolved ``project_id``
        (project subscribers subscribe at project_id scope), not under
        ``task_id`` (fix #1).
        """
